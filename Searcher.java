...
public abstract class Searcher<T> {
	 ...
	 * 
	 * A valid solution consists of a list of states.
	 * The list should start with the initial state of the underlying problem.
	 * Then, it should have one or more additional states.
	 * Each state should be a successor of its predecessor.
	 * The last state should be a goal state of the underlying problem.
	 * 
	 * @param solution
	...
	public final boolean isValidSolution(List<T> solution) {
		...
		if (!solution.get(0).equals(searchProblem.getInitialState()) || !searchProblem.getSuccessors(solution.get(0)).contains(solution.get(1)) || !searchProblem.isGoal(solution.get(solution.size()-1))) {
			return false;
		}
